    ...
    for subject in range(num_subjects):
        for game_played in range(1):
            ...
            for trial in range(num_trials):
                ...
                action_values = np.exp((q_integrated)-sf.logsumexp(q_integrated));
                values=action_values.flatten()
                # print('qvalues at time of choice:{}'.format(q_values))
                # print('action probs after softmax at time of choice:{}'.format(values))
                # print('RF at time of choice: {}'.format(current_reward_function))
                draw = choice(values,1,p=values)[0]
                indices = [i for i, x in enumerate(values) if x == draw]
                current_choice=sample(indices,1)[0]
                # print('current choice= {}'.format(current_choice))
                qv1.append(q_values[0])
                qv2.append(q_values[1])
                ...
                #determine current outcome by feature vector * reward function
                current_outcome=sum(current_features*current_reward_function)
                # print('outcome after choice: {}'.format(current_outcome))
                #save how distant estimated EV is from actual EV 
                q_rews.append(np.abs(current_bandit-(q_sr[current_choice])))
                outcomes.append(current_outcome)
                ...
                feature_matrix[current_decision,1]=feature_matrix[current_decision,1]+(lr_features[1]*pe_features[current_decision,1])
                feature_matrix[other_decision,1]=feature_matrix[other_decision,1]+(lr_cf_r*pe_features[other_decision,1])
                # print('q value of current choice: {}'.format(q_values[current_choice]))
                # print('prediction error={}'.format(lr_value*(current_outcome-q_values[current_choice])))
                #Model-Free action value updating
                q_values[current_choice] = q_values[current_choice]+ (lr_value*(current_outcome-q_values[current_choice]))
                # print('updated q values: {}'.format(q_values))
            #save all output
            q_rew_all.append(np.sum(q_rews))
            all_outcomes.append(np.sum(outcomes))
            diffqvalues=[qv1[i]-qv2[i] for i in range(len(qv1))]
            # df=pd.DataFrame()
            # df['action_values']=diffqvalues+rfs[:-1]
            # df['labels']=['diff qvalues']*160+['goal']*160
            # df['trial']=list(np.arange(160)+1)+list(np.arange(160)+1)
            # sns.scatterplot(data=df,x='trial',y='action_values',hue='labels')
            # plt.show()
...
lr_cfr=list(np.zeros((num_subjects))+0.10)
lr_cfp=list(np.zeros((num_subjects))+0.10)


betas=np.zeros((2,num_subjects))

beta_safe=list(np.zeros((num_subjects))+5)
beta_pred=list(np.zeros((num_subjects))+5)
betas[0]=beta_safe
betas[1]=beta_pred
...
reward_function_timeseries=np.concatenate((rf1a,rf2a),axis=0)
reward_function_timeseries2=np.concatenate((rf1,rf2),axis=0)
reward_function_timeseries3=np.concatenate((rf2,rf1),axis=0)
reward_function_timeseries4=np.concatenate((rf2a,rf1a),axis=0)
...
all_data4,outcomes4,rewards4,choices4,safes4,preds4=simulate_one_step_featurelearner_counterfactual_2states_2iRF_sticky(num_subjects,num_trials,lrf2,lrv,betas,mf_betas,bandits,reward_function_timeseries2,pred_change,rew_change,lr_cfr,lr_cfp,rfi_rews,rfi_preds,stick)
all_data5,outcomes5,rewards5,choices5,safes5,preds5=simulate_one_step_featurelearner_counterfactual_2states_2iRF_sticky(num_subjects,num_trials,lrf2,lrv,betas,mf_betas,bandits,reward_function_timeseries3,pred_change,rew_change,lr_cfr,lr_cfp,rfi_rews,rfi_preds,stick)
all_data6,outcomes6,rewards6,choices6,safes6,preds6=simulate_one_step_featurelearner_counterfactual_2states_2iRF_sticky(num_subjects,num_trials,lrf2,lrv,betas,mf_betas,bandits,reward_function_timeseries4,pred_change,rew_change,lr_cfr,lr_cfp,rfi_rews,rfi_preds,stick)
...
sns.set(style='white', font='Arial', font_scale=1.5, color_codes=True, rc=None)
...
dataf['alphas']=alphas
ax = sns.barplot(x="trial_types", y="percent_switch",hue="trial_type", data=dataf,edgecolor="black",palette=["dodgerblue","navy"])
...
plt.savefig('MB1',bbox_inches='tight',dpi=300)
plt.show()
...
ax.set_xlabel('')
plt.ylim([0,0.70])
plt.savefig('MB2',bbox_inches='tight',dpi=300)
plt.show()
...
beta_safe=list(np.zeros((num_subjects))+0)
beta_pred=list(np.zeros((num_subjects))+0)
betas[0]=beta_safe
betas[1]=beta_pred
#np.zeros((num_subjects))+3.0 # learn about predator feature
...
mf_betas=list(np.zeros((num_subjects))+0)
# beta_irf=list(gammarnd(0.29,3.92,num_subjects))
rfi_rews=list(np.zeros((num_subjects))+5)
rfi_preds=list(np.zeros((num_subjects))+5)
stick=list(np.zeros((num_subjects))+0)
...
for data_mf in current_dataset:
    ...
    for trial in range(start,len(data_mf)):
        ...
        last_o_rew_other.append(data_mf_red['safes'][trial][other_state-1])
        last_o_rew.append(data_mf_red['safes'][trial][chosen_state-1])
        safe_rf.append(abs(data_mf_red['rf'][trial][1]))
...
dataf['trial_type']=trial_type
dataf['alphas']=alphas
ax = sns.barplot(x="trial_types", y="percent_switch",hue="trial_type", data=dataf,edgecolor="black",palette=["mediumseagreen","darkgreen"])
handles, labels = ax.get_legend_handles_labels()
ax.legend(handles=handles[0:], labels=labels[0:])
ax.set(xticklabels=[])
ax.set_ylabel('')    
ax.set_xlabel('')
plt.ylim([0,0.70])
plt.savefig('GP1',bbox_inches='tight',dpi=300)
plt.show()
...
ax.set_xlabel('')
plt.ylim([0,0.70])
plt.savefig('GP2',bbox_inches='tight',dpi=300)
plt.show()
...
mf_betas=list(np.zeros((num_subjects))+5)
# beta_irf=list(gammarnd(0.29,3.92,num_subjects))
rfi_rews=list(np.zeros((num_subjects))+0)
rfi_preds=list(np.zeros((num_subjects))+0)
stick=list(np.zeros((num_subjects))+0)
...
dataf['trial_type']=trial_type
dataf['alphas']=alphas
ax = sns.barplot(x="trial_types", y="percent_switch",hue="trial_type", data=dataf,edgecolor="black",palette=["salmon","darkred"])
handles, labels = ax.get_legend_handles_labels()
ax.legend(handles=handles[0:], labels=labels[0:])
ax.set(xticklabels=[])
...
ax.set_xlabel('')
plt.ylim([0,0.70])
plt.savefig('MF1',bbox_inches='tight',dpi=300)
plt.show()
...
ax.set_xlabel('')
plt.ylim([0,0.70])
plt.savefig('MF2',bbox_inches='tight',dpi=300)
plt.show()