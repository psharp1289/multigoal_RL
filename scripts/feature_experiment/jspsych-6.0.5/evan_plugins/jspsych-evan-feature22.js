...
jsPsych.plugins["evan-feature22"] = (function() {
  ...
  plugin.trial = function(display_element, trial) {
    ...
    var c_f_outcomes = [c1_f_outcomes, c2_f_outcomes];
    var c_rewards = [c1_reward, c2_reward];
    ...
    var display_choice_stim_wait_for_response = function(){
      if (!trial.single_choice_option){
        ...
        // middle image
        d3.select("svg").append("svg:image").attr("class", "choice cR").attr("x", choice_image_x_arr[1])
        ...
        if (trial.single_choice_option){
          var valid_responses = [choose_right_key];
        }else{
