...
def feature_learner_lik_beta_counterfactual_two_empirical_2lr_rfinsensitive_sticky_noMB(samples,data,rng_samples):
	...
	pred_changer=samples[4][rng_samples]
	rew_changer=samples[5][rng_samples]
	lr_features=samples[1][rng_samples]
	lr_features=lr_features.reshape(sample_size,1)
	rfi_rew=samples[6][rng_samples]
	rfi_pred=samples[7][rng_samples]
...
def feature_learner_lik_beta_counterfactual_two_empirical_2lr_rfinsensitive_gradsticky(samples,data,rng_samples):
	from scipy.special import logsumexp
	sample_size=len(rng_samples)
	...
	beta_last=samples[10][rng_samples]
	beta_last=beta_last.reshape(sample_size,1)
	lr_stick=samples[11][rng_samples]
	...
	for index in range(len(choice)):
		...
		pe_stick=1-last_choice[:,current_decision]
		last_choice[:,current_decision]=last_choice[:,current_decision]+(lr_stick*pe_stick)
		last_choice[:,other_decision]=1-last_choice[:,current_decision]
		...
		q_values[:,int(choice[index])-1]=q_values[:,int(choice[index])-1]+ (lr_val*pe)
		# q_values[:,other_decision]=q_values[:,int(choice[index])-1]*(1-lr_val)
...
def feature_learner_lik_beta_counterfactual_two_empirical_2lr_rfinsensitive_sticky_mfdecay(samples,data,rng_samples):
	from scipy.special import logsumexp
	sample_size=len(rng_samples)
...
def feature_learner_lik_beta_counterfactual_two_empirical_2lr_rfinsensitive_sticky_2LRMF(samples,data,rng_samples):
	from scipy.special import logsumexp
	sample_size=len(rng_samples)
	...
	pred_changer=samples[5][rng_samples]
	rew_changer=samples[6][rng_samples]
	lr_val_pos=samples[4][rng_samples]
	lr_features=samples[1][rng_samples]
	lr_features=lr_features.reshape(sample_size,1)
	betas_safe=samples[2][rng_samples]
	betas_pred=samples[3][rng_samples]
	...
	beta_last=samples[10][rng_samples]
	beta_last=beta_last.reshape(sample_size,1)
	lr_val_neg=samples[11][rng_samples]
	# order_changer=samples[9][rng_samples]
	# beta_rfinsensitive_r=samples[9][rng_samples]
	# betas_rfinsensitive_r=beta_rfinsensitive_r.reshape(sample_size,1)
	mf_betas=samples[0][rng_samples]
	...
	for index in range(len(choice)):
		...
		if -1 in current_reward_function:
			q_values[:,int(choice[index])-1]=q_values[:,int(choice[index])-1]+ (lr_val_neg*pe)
		else:
			q_values[:,int(choice[index])-1]=q_values[:,int(choice[index])-1]+ (lr_val_pos*pe)
...
def feature_learner_lik_beta_counterfactual_two_empirical_2lr_rfinsensitive_sticky_noMF(samples,data,rng_samples):
	...
	pred_q=np.zeros((sample_size,num_choices))
	lik=np.zeros((sample_size,))
	lr_cf=samples[7-2][rng_samples]
	lr_cf=lr_cf.reshape(sample_size,1)
	pred_changer=samples[5-2][rng_samples]
	rew_changer=samples[6-2][rng_samples]

	lr_features=samples[1-1][rng_samples]
	lr_features=lr_features.reshape(sample_size,1)
	betas_safe=samples[2-1][rng_samples]
	betas_pred=samples[3-1][rng_samples]
	rfi_rew=samples[8-2][rng_samples]
	rfi_pred=samples[9-2][rng_samples]
	beta_last=samples[10-2][rng_samples]
	beta_last=beta_last.reshape(sample_size,1)
...
def feature_learner_lik_beta_counterfactual_two_empirical_2lr_rfinsensitive_sticky_Psensitivity(samples,data,rng_samples):
	...
	if reverse=='no':
		...
		safes=[[float(data.s_1_f_1_outcome[i]),float(data.s_2_f_1_outcome[i])] for i in range(len(data.s_2_f_1_outcome))]
		predators=[[float(data.s_1_f_2_outcome[i]),float(data.s_2_f_2_outcome[i])] for i in range(len(data.s_2_f_1_outcome))]
		psens_rfi=np.concatenate((np.zeros((sample_size,1)),p_sens),axis=1)
		...
		safes=[[float(data.s_1_f_2_outcome[i]),float(data.s_2_f_2_outcome[i])] for i in range(len(data.s_2_f_1_outcome))]
		predators=[[float(data.s_1_f_1_outcome[i]),float(data.s_2_f_1_outcome[i])] for i in range(len(data.s_2_f_1_outcome))]
		psens_rfi=np.concatenate((p_sens,np.zeros((sample_size,1))),axis=1)
	...
	for index in range(len(choice)):
		...
		rf_insensitive_weights=np.concatenate((rfi_rew.reshape(sample_size,1),rfi_pred.reshape(sample_size,1)*-1),axis=1)
		q_rfi1=np.sum((feature_matrix1*rf_insensitive_weights*psens_rfi),axis=1) 
		q_rfi2=np.sum((feature_matrix2*rf_insensitive_weights*psens_rfi),axis=1)
		q_GP=np.concatenate((q_rfi1.reshape(sample_size,1),q_rfi2.reshape(sample_size,1)),axis=1)
		# +q_rfi_rew add in if want reward insensitive
...
def feature_learner_lik_beta_counterfactual_two_empirical_justLR(samples,data,rng_samples):
	...
	pred_q=np.zeros((sample_size,num_choices))
	lik=np.zeros((sample_size,))
	lr_cf=samples[2][rng_samples]
	lr_cf=lr_cf.reshape(sample_size,1)
	# pred_changer=samples[5][rng_samples]
	# rew_changer=samples[6][rng_samples]

	lr_safe=samples[0][rng_samples]
	lr_safe=lr_safe.reshape(sample_size,1)
	lr_pred=samples[3][rng_samples]
	lr_pred=lr_pred.reshape(sample_size,1)
	betas=samples[1][rng_samples]
	lr_val=samples[5][rng_samples]
	mf_betas=samples[4][rng_samples]
	mf_betas=mf_betas.reshape(sample_size,1)
...
def feature_learner_lik_beta_counterfactual_two_empirical_justLR_change(samples,data,rng_samples):
	from scipy.special import logsumexp
	sample_size=len(rng_samples)
...
# name_1='feature_learner_winning_noMF'
...
# all_models.append(feature_learner_winning_noMF)
...
build_model(name_1,likelihood,group_parameters_info_m,number_subjects,parameter_sample_size)
all_models.append(mb_rr_ap_mf_SINGLEgp)
...
for model in all_models:
	...
	while improvement>0:
		...
		for subject in range(number_subjects):
		   ...
		   two.append(all_results[subject][2][1]) #mfb
		   lr_neg.append(all_results[subject][3][1])#sticky
		   beta_s.append(all_results[subject][4][1]) #lr feature
		   beta_p.append(all_results[subject][5][1]) #beta s 
		   lr_val.append(all_results[subject][6][1]) # beta p
		   pre_change.append(all_results[subject][7][1]) #lr cf
		   re_change.append(all_results[subject][8][1]) #pre change
		   lr_cf.append(all_results[subject][9][1]) #lr val
		   beta_rfis_r.append(all_results[subject][10][1])  #re change
		   beta_rfis_p.append(all_results[subject][11][1])#rfi r
		   beta_st.append(all_results[subject][12][1])# rfi p
		   # mfd.append(all_results[subject][11][1])# decay
