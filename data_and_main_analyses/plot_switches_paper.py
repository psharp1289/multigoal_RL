...
for data_mf in current_dataset:
	...
	last_predrf=[0]+pred_rf
	last_predrf=last_predrf[:-1]
	last_saferf=[0]+safe_rf
	last_saferf=last_saferf[:-1]
	...
	if both_pred>1:
		...
		X_vars['last_rew']=np.array(last_o_rew)
		X_vars['last_pred']=np.array(last_o_pred)
		...
		X_vars['pred_rf']=np.array(pred_rf)
		X_vars['last_pred_rf']=np.array(last_predrf)# -1 if safe
		...
		annex[0:missing_rows,1]=np.round(np.mean(last_o_rew))
		annex[0:missing_rows,2]=np.round(np.mean(last_o_pred))
		annex[0:missing_rows,3]=np.round(np.mean(last_o_rew_other))
		annex[0:missing_rows,4]=np.round(np.mean(last_o_pred_other))
		annex[0:missing_rows,5]=np.round(np.mean(rewmb))
		annex[0:missing_rows,6]=np.round(np.mean(punmb))
		annex[0:missing_rows,7]=np.round(np.mean(rewmf))
		annex[0:missing_rows,8]=np.round(np.mean(punmf))
		annex[0:missing_rows,9]=np.round(np.mean(rewmb_cf))
		annex[0:missing_rows,10]=np.round(np.mean(punmb_cf))
...
sns.set(style='white', palette='deep', font='arial', font_scale=0.8, color_codes=True, rc=None)
...
diff_pun=[np_nrfsP[i]-np_psP[i] for i in range(len(Bp_nrfsP))]
diff_rwd=[np_nrfs[i]-np_ps[i] for i in range(len(Bp_nrfsP))]
t,p=ttest_rel(diff_pun,diff_rwd)
print('T TEST #1: {}, pval: {}'.format(t,p))
mean_both_none=np.mean([Bnp_psP[i]+Bnp_ps[i]+p_nrfsP[i]+p_nrfs[i] for i in range(len(np_ps))])
mean_both_both=np.mean([Bp_nrfsP[i]+Bp_nrfs[i]+np_psP[i]+np_ps[i] for i in range(len(np_ps))])
...
# box_pairs = [
#     (("NoPred_Rew", "PUN \u2192 PUN"), ("NoPred_NoRew", "PUN \u2192 PUN")),
#     (("NoPred_Rew", "PUN \u2192 RWD"), ("NoPred_NoRew", "PUN \u2192 RWD")),
#     (("Pred_Rew", "PUN \u2192 PUN"), ("Pred_NoRew", "PUN \u2192 PUN"))
#     ]

# width = 0.4
# ax = sns.barplot(x='trial_types', y='percent_switch',hue='trial_type', data=dataf,edgecolor="black",palette=["lightgray","dimgray"])
# add_stat_annotation(ax, plot='barplot', x='trial_types', y='percent_switch',hue='trial_type', data=dataf,
#                     box_pairs=box_pairs, test='t-test_paired', loc='inside', verbose=2)
...
# plt.savefig('Empirical_modelagnosticplots_1.png', dpi=300, bbox_inches='tight')
# plt.show()

diff_pun=[Bnp_nrfsP[i]-Bp_nrfsP[i] for i in range(len(Bp_nrfsP))]
diff_rwd=[Bnp_nrfs[i]-Bp_nrfs[i] for i in range(len(Bp_nrfsP))]
t,p=ttest_rel(diff_pun,diff_rwd)
print('T TEST #2: {}, pval: {}'.format(t,p))
x_col=[]
y_col=[]
trial_type=[]
alphas=[]
...
dataf['alphas']=alphas
box_pairs = [
    (("NoPred_Rew", "RWD \u2192 RWD"), ("Pred_Rew", "RWD \u2192 RWD")),
    (("NoPred_NoRew", "RWD \u2192 RWD"), ("Pred_NoRew", "RWD \u2192 RWD")),
    (("NoPred_Rew", "RWD \u2192 PUN"), ("Pred_Rew", "RWD \u2192 PUN")),
...
ax.set_xlabel('')
plt.legend(bbox_to_anchor=(1.05, 1), loc=2, borderaxespad=0.)
plt.savefig('Empirical_modelagnosticplots_2.png', dpi=300, bbox_inches='tight')
plt.show()
...
print(np.mean(pps),np.std(pps))
print(np.mean(p_nrfs),np.std(p_nrfs))
print(np.mean(np_ps),np.std(np_ps))
print(np.mean(np_nrfs),np.std(np_nrfs))
print('')
print(np.mean(ppsP),np.std(ppsP))
print(np.mean(p_nrfsP),np.std(p_nrfsP))
print(np.mean(np_psP),np.std(np_psP))
print(np.mean(np_nrfsP),np.std(np_nrfsP))
print('')


print(np.mean(Bpps),np.std(Bpps))
print(np.mean(Bp_nrfs),np.std(Bp_nrfs))
print(np.mean(Bnp_ps),np.std(Bnp_ps))
print(np.mean(Bnp_nrfs),np.std(Bnp_nrfs))
print('')

print(np.mean(BppsP),np.std(BppsP))
print(np.mean(Bp_nrfsP),np.std(Bp_nrfsP))
print(np.mean(Bnp_psP),np.std(Bnp_psP))
print(np.mean(Bnp_nrfsP),np.std(Bnp_nrfsP))
