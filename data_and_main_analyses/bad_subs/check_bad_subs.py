...
for data_mf in current_dataset:
	...
	if ppsP[0]<0:
		if np_psP[0]>0:
			...
			bad_pattern=1
			pred_wrong+=1
			# print('pred wrong: {} for file {}'.format(violations,subs[sub_num]))
	...
	if pps[0]>0:
		if np_ps[0]<0:
			...
			bad_pattern=1
			rew_wrong+=1
			# print('rew wrong: {} for file {}'.format(violations,subs[sub_num]))
	...
	if ppsP[0]<0:
		if np_psP[0]>0:
			if pps[0]>0:
				if np_ps[0]<0:
					print('both rew and pun wrong: {} for file {}'.format(violations,subs[sub_num]))

					violations+=10 #both are incorrect, delete

	if rew_wrong==1 and pred_wrong==1:
		print('both rew and pun wrong: {} for file {}'.format(violations,subs[sub_num]))
...
print('percent bad subjects: {}'.format(total_violations/len(subs)))
