...
gprew=np.load('gprews_empirical4.npy')

gppun=np.load('gppuns_empirical4.npy')


mbrew=np.load('mbrews_empirical4.npy')


mbpun=np.load('mbpuns_empirical4.npy')

mfrew=np.load('mfrews_empirical4.npy')


mfpun=np.load('mfpuns_empirical4.npy')
...
# print(axs[0,0].get_lines())
# x,y = axs[0,0].get_lines()[0].get_data()
# y=list(y)
# x=list(x)
# mode= max(y)
...
plt.savefig('logisticregression_switchtrialsf_empirical_cleaned_ver2.png', bbox_inches='tight',  dpi=300)
...
for i in mfdiff:
    if i<0:
        z+=1
print('PD MF DIFF MAIN effect = {}'.format(1-((8000-z)/8000.0)))
print('')
...
axs[2].set_xlim([-0.8, 0.80])
plt.subplots_adjust(top = 0.99, bottom=0.01, hspace=0.4, wspace=0.18)
f.text(0.5, -.12, 'Parameter Estimate', ha='center')
f.text(0.0, 0.5, 'Density', va='center', rotation='vertical')
plt.savefig('logisticregression_switchtrials_diffs_empirical_cleaned_ver2.png', bbox_inches='tight',  dpi=300)
plt.show()
...
f.text(0.5, -.12, 'Parameter Estimate', ha='center')
f.text(0.0, 0.5, 'Density', va='center', rotation='vertical')
plt.savefig('logisticregression_switchtrials_maineffects_cleaned_empirical_ver2.png', bbox_inches='tight',  dpi=300)
